...
DirectImputationOffsetBiv <- function(formula.fixed, formula.random, formula.imp, 
  ...
  # output
  out <- list(new.VarCov = VarCov.Mat.Random, new.se = SE, est.new = b.new, lme.imp = mod.imp)
  return(out)
}
...
## INPUT:   formula.fixed (formula for the fixed effects in our model of interest), formula.random
##          (formula for the random effects in our model of interest). formula.imp 
##          (formula for the logistic model used for impuation), data (dataset), n.imp (number of imputations),
...
IIM <- function(formula.fixed, formula.random, formula.imp, data, n.imp, n.burn, id,
                          grp, timegrp, time, yname){
  if(n.burn >= n.imp){stop("Burn needs to be less than the number of replicates")}
  ...
  coeff.trace <- do.call(rbind, Ests.Imp)
  Ests.Imp    <- Ests.Imp[-c(1:n.burn)]
  Covs.Imp    <- Covs.Imp[-c(1:n.burn)]
  # put results together
  coeff.res <- MIcombine(Ests.Imp, Covs.Imp)
  out <- list(coefficients = coeff.res$coefficients, covariance = diag(coeff.res$variance), 
              coeff.trace = coeff.trace)
  return(out)
}
