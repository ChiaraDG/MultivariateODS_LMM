...
simulation <- function(run, count)
{
  ...
  # Re-Use data
  dat.samp <- SampledFunReuse(d = dat, Nsample = Nsample.sim, NsPerStratumUniv = NsPerStratumUniv.sim, 
                          Y1.sample = TRUE, Y2.sample = FALSE, 
                          Y1.cutoffsODS =  cutoffs.Y1.ODS, Y2.cutoffsODS = cutoffs.Y2.ODS,
                          Y1.cutoffsBLUP =  cutoffs.Y1.BLUP, Y2.cutoffsBLUP = cutoffs.Y2.BLUP)
  ...
  dat.long$SlpProbLow <- dat.samp$SampProbs[[1]][1]
  dat.long$SlpProbMed <- dat.samp$SampProbs[[1]][2]
  dat.long$SlpProbHig <- dat.samp$SampProbs[[1]][3]
  ...
  # fit model
  acml.slp <- acml.lmem2(formula.fixed = Y ~ 0 + D1 + g1 + c1 + t1 + gt1 + ct1 + D2 + g2 + c2 + t2 + gt2 + ct2,
                            formula.random = ~ 0 + D1 + t1 + D2 + t2,
                            data = dat.long, id = id, InitVals = truevals.biv,
                            SampProb = SampProbSlp, cutpoints = cutpointsSlp, Weights = NoWeighting, w.function = w.functionSlp)

  acml.slp  <- list(coefficients = acml.slp$coefficients[1:12], 
                    covariance = diag(acml.slp$covariance)[1:12])
  ...
  results <- cbind(Sampling = c("Y2 Only", "ACML Slope", "ODSSlope"), 
