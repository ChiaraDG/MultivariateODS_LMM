...
vars     <- function(fits)
{
  result <- t(sapply(fits, function(f) f$variance[1:42]))
...
SampledFun <- function(d){
  ...
  #### BDS Sampling ####
  StratSlpBDS       <- identify.stratum(w.function="slope", cutpoints = cutpoints.BDS.Y1, 
                                        Slp = random.slope.Y1)
  nobs              <- d %>% group_by(id) %>% summarise(nobs = n()) %>% pull(nobs)
  d$StratSlpBDS.Y1  <- rep(StratSlpBDS, times = nobs)
  
  StratSlpBDS       <- identify.stratum(w.function="slope", cutpoints = cutpoints.BDS.Y2, 
                                         Slp = random.slope.Y2)
  d$StratSlpBDS.Y2  <- rep(StratSlpBDS, times = nobs)
  ...
  # BLUP sampling
  SampledSlpBDS      <- ods.sampling(id.long = dY1$id, stratum.long = dY1$StratSlpBDS.Y1, 
                                     SamplingStrategy = "IndepODS", NsPerStratum = NsPerStratumUniv)
  dY1$SampledSlpBDS  <- SampledSlpBDS$Sampled
  ...
  # BLUP sampling
  SampledSlpBDS   <- ods.sampling(id.long = dY2$id, stratum.long = dY2$StratSlpBDS.Y2, 
                               SamplingStrategy = "IndepODS", NsPerStratum = NsPerStratumUniv)
  dY2$SampledSlpBDS <- SampledSlp$Sampled
...
simulation <- function(dataset = dat, run, count)
{
  ...
  # save results
  acml.slp  <- list(coefficients = acml.slp$coefficients[1:42], variance = diag(acml.slp$covariance)[1:42],
                     covariance = c(acml.slp$covariance[2, 11], acml.slp$covariance[2, 9],
                                    acml.slp$covariance[2, 10], acml.slp$covariance[23, 32],
                                    acml.slp$covariance[23, 30], acml.slp$covariance[23, 31]))
  ...
  model.rs   <- ImputationBiv(d = fulldat, imp = 650, burn = 50, sampling = "random")
  model.ods  <- ImputationBiv(d = fulldat, imp = 650, burn = 50, sampling = "SlpODS")
  model.bds  <- ImputationBiv(d = fulldat, imp = 650, burn = 50, sampling = "SlpBLUP")
 
  
  # get the results
  cat("Saving Results")
  fits <- list(CompleteCase.MLE, acml.slp, acml.bds, model.rs, model.ods, model.bds)

  results <- cbind(Sampling = c("Random","ACML ODS", "ACML BDS", "Random IIA", "ODS IIA", "BDS IIA"), 
                   ests(fits), vars(fits), covars(fits))
  # save the results
